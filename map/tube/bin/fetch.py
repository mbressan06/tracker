...
def canon_station_name(s, line):
    ...
    s = s.strip()
    s = re.sub('^Camden Town at Point 20A Station$', 'Camden Town at Point Station', s)
    s = re.sub("^Lord's Disused Station$", "Lords Disused Station", s)
    ...
    s = re.sub('^Earl\u2019s Court Station', "Earls Court Station", s)
    s = re.sub('^Camden Town at Point 20A Station$', 'Camden Town at Point Station', s)
    ...
    s = s.replace('(Bakerloo)', 'Bakerloo').replace('Earls', 'Earl\'s') \
        ...
        .replace("Lord's Disused Station", "Lord's Disused Station") \
...
for line, ids in out.items():
    for id, arr in ids.items():
        ...
        if "Lord's Disused Station" in arr['current_location']: continue
        ...
        if m:
            ...
            location_2 = station_locations[station_name.encode('ascii')]
